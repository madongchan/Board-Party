...
public class IdleState : NPCState
{
    ...
    {
        UIManager.Instance.ShowNPCState($"{npcController.gameObject.name} State : {this.GetType().Name}");
...
public class TurnEndState : NPCState
{
    ...
    public override void Enter()
    {
        ...
        npcController.OnTurnEnd.AddListener(OnTurnEnd);
    }

    private void OnTurnEnd()
    {
        // 턴 종료 시 필요한 처리
        Debug.Log($"{npcController.name}의 턴이 종료되었습니다.");
        GameManager.Instance.EndCurrentTurn();
        npcController.OnTurnEnd.RemoveListener(OnTurnEnd);
    }
    public override void Update()
    {
...
public class StarPurchaseDecisionState : NPCState
{
    ...
    public override void Enter()
    {
        ...
        // 별 구매 UI는 StarSpace에서 처리하므로 여기서는 별도 처리 없음
        // StarSpace가 NPC 타입을 감지하고 자동으로 처리함
    }
